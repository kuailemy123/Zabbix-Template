...
CACHE_STATS_EXPIRATION="${CACHE_STATS_EXPIRATION:-60}" # in seconds
CACHE_INFO_FILEPATH="${CACHE_INFO_FILEPATH:-/var/tmp/haproxy_info.cache}" ## unused
CACHE_INFO_EXPIRATION="${CACHE_INFO_EXPIRATION:-60}" # in minutes ## unused
GET_STATS=${GET_STATS:-1} # when you update stats cache outsise of the script
SOCAT_BIN="$(which socat)"
NC_BIN="$(which nc)"
FLOCK_BIN="$(which flock)"
FLOCK_WAIT=5 # maximum number of seconds that "flock" waits for acquiring a lock
FLOCK_SUFFIX='.lock'
...
check_cache() {
  ...
  if [ $((cache_filemtime+cache_expiration)) -ge ${CUR_TIMESTAMP} ]
  then
